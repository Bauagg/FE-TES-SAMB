...
const PenerimaanBarangDetail = () => {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_API_URL}/api/transaksi-penerima-barang-detail`)
            .then((result) => {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_API_URL}/api/transaksi-penerima-barang-header`)
            .then((result) => {
    ...
    const createPenerimaanDetail = () => {
        const data = {
            trx_in_d_product_idf: parseInt(trx_out_d_product_idf),
            trx_in_d_qty_dus: parseInt(trx_out_d_qty_dus),
            trx_in_d_qty_pcs: parseInt(trx_out_d_qty_pcs),
            trx_in_idf: parseInt(trx_out_idf)
        }

        axios.post(`${process.env.REACT_APP_API_URL}/api/transaksi-penerima-barang-detail`, data)
            .then(() => {
                axios.get(`${process.env.REACT_APP_API_URL}/api/transaksi-penerima-barang-detail`)
                    .then((result) => {
    ...
    const updatePenerimaanDetail = () => {
        ...
        axios.put(`${process.env.REACT_APP_API_URL}/api/transaksi-penerima-barang-detail/${idData}`, data)
            .then(() => {
                axios.get(`${process.env.REACT_APP_API_URL}/api/transaksi-penerima-barang-detail`)
                    .then((result) => {
    ...
    return (
        <div className="mx-4">
            <h1 className="font-bold fon text-[30px] text-center mb-5">Penerimaan Barang Detail</h1>
            <button onClick={() => setToggleCreate(true)} className="bg-blue-500 hover:bg-blue-600 text-white py-1 px-3 rounded">Create Penerimaan Barang Detail</button>
            <div>
                <table className="min-w-full bg-white border border-gray-200 shadow-md rounded">
                    <thead>
                        <tr>
                            <th className="py-2 px-4 border-b">ID</th>
                            <th className="py-2 px-4 border-b">Trx In No</th>
                    ...
                    <tbody>
                        {data.map((item, index) => (
                            <tr key={index} className="text-center">
                                ...
                                <td className="py-2 px-4 border-b">{item.Header.trx_in_no}</td>
                                ...
                                <td className="py-2 px-4 border-b">{item.trx_in_d_qty_dus}</td>
                                <td className="py-2 px-4 border-b">{item.trx_in_d_qty_pcs}</td>
                ...
                <div className="fixed inset-0 bg-gray-500 bg-opacity-50 flex justify-center items-center">
                    <div className="bg-white p-6 rounded shadow-lg w-96">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-lg font-semibold">Create Penerimaan Barang Detail</h2>
                        ...
                        <div>
                            ...
                            <div className="mb-4">
                                <label className="block text-sm font-medium text-gray-700">Trx In Idf</label>
                                <select
                                    value={trx_out_idf}
                                    ...
                                    className="mt-1 py-2 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                >
                                    <option value="">Select Trx In Idf</option>
            ...
            {toggleUpdate && (
                <div className="fixed inset-0 bg-gray-500 bg-opacity-50 flex justify-center items-center">
                    <div className="bg-white p-6 rounded shadow-lg w-96">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-lg font-semibold">Update Penerimaan Barang Detail</h2>
...
export default PenerimaanBarangDetail